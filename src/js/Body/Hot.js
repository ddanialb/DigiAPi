...
async function Hot() {
  try {
    const data = await fetchAPI("trending.json");

    if (!data.data || !data.data.products) {
      console.error("Hot products not found");
      return;
    }
    ...
    if (hotWrapper) {
      hotWrapper.innerHTML = desktopSlides;
    }
    ...
    const hotMobileWrapper = document.querySelector(".hot-mobile-wrapper");
    if (hotMobileWrapper) {
      hotMobileWrapper.innerHTML = mobileSlides;
    }
  } catch (error) {
